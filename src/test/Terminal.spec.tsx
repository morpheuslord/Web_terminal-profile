...
describe("Terminal Component", () => {
  ...
  describe("Redirect commands", () => {
    ...
    nums.forEach(num => {
      it(`should redirect to research URL when user type 'research go ${num}' cmd`, async () => {
        await user.type(terminalInput, `research go ${num}{enter}`);
        expect(window.open).toHaveBeenCalled();
      });
    });
  ...
  describe("Invalid Arguments", () => {
    const specialUsageCmds = ["themes", "socials", "projects", "research"];
    ...
    specialUsageCmds.forEach(cmd => {
      ...
      it(`should return usage component for '${cmd}' cmd with incorrect option`, async () => {
        ...
        await user.type(terminalInput, `research go 4{enter}`);
