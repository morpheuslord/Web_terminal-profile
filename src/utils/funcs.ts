...
export const argTab = (
  ...
  else if (inputVal === "projects " || inputVal === "socials " || inputVal === "research") {
    setInputVal(`${inputVal}go`);
    return [];
  }

  // 6) if input is 'projects g' or 'socials g'
  else if (inputVal === "projects g" || inputVal === "socials g" || inputVal === "research g") {
  ...
  // 7) if input is 'socials go '
    ...
    ["1.Github", "2.Hackernoon", "3.LinkedIn", "4.Twitter"].forEach(t => {
      hintsCmds = [...hintsCmds, t];
    });
    return hintsCmds;
  }
  ...
  // 8) if input is 'projects go '
    ...
    [
      "1.Brute-Hacking-Framework-SourceCode",
      "2.Nmap-API",
      "3.PassInfo",
      "4.GPT-Vuln_Analyzer",
    ].forEach(t => {
  ...
  // 9) if input is 'research go '
  else if (_.startsWith(inputVal, "research go ")) {
    [
      "1.AI Based Enumeration and Exploit Suggester ",
    ].forEach(t => {
      hintsCmds = [...hintsCmds, t];
    });
    return hintsCmds;
